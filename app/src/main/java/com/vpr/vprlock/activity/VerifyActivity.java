...
public class VerifyActivity extends BaseActivity {
    ...
    TextView hintTv;
    @ViewInject(id = R.id.pwdTv)
    TextView pwdTv;
    @ViewInject(id = R.id.centerTv)
    TextView centerTv;
    @ViewInject(id = R.id.leftVolumeView)
    VolumeView leftVolumeView;
    @ViewInject(id = R.id.rightVolumeView)
    VolumeView rightVolumeView;
    @ViewInject(id = R.id.result_icon)
    ImageView resultImage;
    @ViewInject(id = R.id.leftbtn, click = "back")
    Button backBtn;
    @ViewInject(id = R.id.recorderBtn, click = "record")
    Button recordBtn;
    ...
    private VerifierListener listener = new VerifierListener() {
        ...
        public void onResult(VerifierResult result) {
            ...
            if (result.ret == 0) {
                ...
                // 验证不通过
                verifyFail(result.err);
            }
        }

        private void verifyFail(int errorCode) {
            resultImage.setVisibility(View.VISIBLE);
